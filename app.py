...
def excel_to_coord(cell_ref):
    """Convert Excel label (e.g. 'B3') to (row, col) zero-based."""
...
def coord_to_excel(y, x):
    """Convert (row, col) zero-based to Excel label."""
    col = x + 1
    letters = []
    while col > 0:
...
def expand_range(start_ref, end_ref):
    """Expand a straight Excel range to a list of (y, x) tuples."""
...
# Parser
# ---------------------------------------------------------------------
def parse_input_auto(text, grid):
    """Detect Excel or rotation input."""
    import re
    s = text.strip()
    if not s:
...
def rotations_to_coords(start_cell, start_dir, rotations):
    """Build coordinate sequence from start cell, direction, and turns."""
    y, x = start_cell
...
def validate_paths(grid, day_paths, start_cell, start_dir, max_days, max_distance, mode):
    """Validate all daily paths, return (ok, msg, plastic_by_day, distance_by_day, steps_by_day)."""
    ...
    for d, day in enumerate(day_paths, start=1):
        ...
        if not (0 <= y0 < rows and 0 <= x0 < cols):
            return False, f'Dag {d} start buiten raster: {coord_to_excel(y0, x0)}.', [], [], []
    ...
    return True, f'Route is geldig vanaf {start_excel}.', plastic_by_day, distance_by_day, distance_by_day_steps

# ---------------------------------------------------------------------
# Visualisatie
# ---------------------------------------------------------------------
def draw_last_frame(grid, day_paths, start_cell, start_dir, plastic_by_day, distance_by_day_steps):
    """Draw grid with final route and Excel labels."""
    import seaborn as sns
    import matplotlib.pyplot as plt
    import matplotlib.patches as patches
    from io import BytesIO
    ...
    start_excel = coord_to_excel(*start_cell)
    ax.set_title(f'Start = {start_excel} | Plastic = {plastic_total} | Distance = {distance_total}',
...
start_row_label = st.selectbox('Start-rij:', row_labels, index=7)     # 8
